...
module.exports = () => ({
  ...
  evaluate: (ctx) => {
    const trades = ctx.earlyBuyerTrades || [];
    if (trades.length === 0) {
    ...
    if (passed) {
      return {
        passed: true,
        reason: `Tổng SOL early buyers ${total.toFixed(2)} < ${max} ✓`,
        data: { totalSol: total, maxTotalSol: max, buyerCount: trades.length },
      };
    }

    // Liệt kê top 3 ví mua nhiều nhất khi block
    const sortedTrades = [...trades].sort((a, b) => (b.solAmount || 0) - (a.solAmount || 0));
    let detail = `🐋 Whale concentration: ${total.toFixed(2)} SOL từ ${trades.length} ví (>= ${max} SOL)`;
